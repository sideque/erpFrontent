...
export function CreateJournalEntryModal({
  ...
  // Per-line validation — used to render inline errors and gate the post button.
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!filled) continue;
    if (!l.accountCode) {
      lineErrors[i] = 'Pick an account';
      continue;
    }
    const acct = accountByCode.get(l.accountCode);
    if (!acct) {
      lineErrors[i] = 'Unknown account';
      continue;
    }
    if (acct.isGroup) {
      lineErrors[i] = 'This is a group account — pick a leaf';
      continue;
    }
    if (l.debit === 0 && l.credit === 0) {
      lineErrors[i] = 'Enter a debit or credit amount';
      continue;
    }
    if (l.debit > 0 && l.credit > 0) {
      lineErrors[i] = 'Either debit or credit, not both';
      continue;
    }
  ...
  const submit = async () => {
    ...
    if (tooFew) {
      toast.error('Add at least 2 filled lines (account + amount)');
      return;
    }
    if (errorCount > 0) {
  ...
  return (
    <Modal
      ...
      subtitle="Record a manual ledger posting. Debits must equal credits."
      width="max-w-5xl"
      footer={
        <>
